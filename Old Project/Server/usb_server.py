...
SCOUT_PORT  = 8765   # tablets POST scouting data here
FORM_PORT   = 8766   # tablets fetch form.html from here

MAX_TABLETS = 6
POLL_INTERVAL = 2    # seconds between adb devices checks
...
connected_tablets: dict[str, datetime] = {}   # serial → connected time
...
def log(tag, msg, color='\033[96m'):
    RST = '\033[0m'
    print(f"\033[90m[{_ts()}]\033[0m {color}[{tag}]\033[0m {msg}")
...
class ScoutHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        pass  # silence default HTTP log; we print our own
...
class FormHandler(BaseHTTPRequestHandler):
    """Serves public/ so tablets get form.html via ADB reverse tunnel."""

    # Map /  →  /form.html  (or index.html as fallback)
    INDEX_FILES = ['form.html', 'scout.html', 'index.html']

    def do_GET(self):
        path = self.path.split('?')[0]   # strip query string

        # Root → serve first available index
        if path in ('', '/'):
...
def _get_online_serials() -> set[str]:
    """Return serials of currently online (authorised) ADB devices."""
    code, out, _ = _run_adb('devices')
    serials = set()
    for line in out.splitlines()[1:]:          # skip header
        line = line.strip()
        if line.endswith('\tdevice'):           # authorised & online
            serials.add(line.split('\t')[0])
    return serials


def _setup_tablet(serial: str):
    """Run both adb reverse tunnels for a newly connected tablet."""
    log('ADB', f'New tablet → {serial}  (setting up tunnels...)')

    for port in (SCOUT_PORT, FORM_PORT):
    ...
    log_ok('ADB', f'Tablet {serial} ready  '
                  f'(scout→:{SCOUT_PORT}  form→:{FORM_PORT})')


def _teardown_tablet(serial: str):
    """Remove reverse tunnels when a tablet is unplugged."""
...
def tablet_watcher():
    """
    Background thread — polls adb devices every POLL_INTERVAL seconds.
    Calls _setup_tablet / _teardown_tablet as devices come and go.
    """
            ...
            with tablets_lock:
                ...
                for serial in current - previous:
                    if len(connected_tablets) >= MAX_TABLETS:
                        log_warn('ADB', f'Max tablets ({MAX_TABLETS}) reached — '
                                        f'ignoring {serial}')
                        continue
                    connected_tablets[serial] = datetime.now()
                    threading.Thread(
                        target=_setup_tablet, args=(serial,),
                        daemon=True, name=f'setup-{serial}'
                    ).start()
                ...
                for serial in previous - current:
                    ...
                    threading.Thread(
                        target=_teardown_tablet, args=(serial,),
                        daemon=True, name=f'teardown-{serial}'
                    ).start()
...
def status_printer():
    """Prints a tablet summary every 30 seconds so you know what's connected."""
    time.sleep(10)
    while True:
        with tablets_lock:
...
if __name__ == '__main__':
    ...
    threading.Thread(target=tablet_watcher, daemon=True,
                     name='tablet-watcher').start()
    threading.Thread(target=status_printer, daemon=True,
                     name='status-printer').start()
