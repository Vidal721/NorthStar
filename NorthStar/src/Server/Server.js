...
// ── CORS (allow NorthStar admin UI on any origin) ─────────────
...
// ── Main Data API (for admin dashboard) ───────────────────

app.post('/api/main', (req, res) => {
  try {
    console.log('--- /api/main CALLED ---');
    console.log('BODY:', req.body);

    const games = readJSON(MAIN_DATA, []);
    console.log('FILE PATH:', MAIN_DATA);
    console.log('FILE DATA:', games);
    console.log('IS ARRAY?', Array.isArray(games));

    const { name, year } = req.body;

    if (!name || !year) {
      console.log('❌ Missing name/year');
      return res.status(400).json({ error: 'Game name and year required' });
    }
    ...
    console.error('💥 ERROR /api/main:', err);
...
app.delete('/api/main/:id', (req, res) => {
  try {
    ...
    console.log(`[DELETE] Game removed: ${id}`);

    res.json({ ok: true });
  } catch (err) {
    console.error('DELETE /api/main error:', err);
    res.status(500).json({ error: err.message });
  }
});
